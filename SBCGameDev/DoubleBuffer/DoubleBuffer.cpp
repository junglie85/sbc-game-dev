...
GLuint CreateTexture2D(int width, int height, char* TheData)
{
    ...
    if (glGetError() != GL_NO_ERROR) printf("Oh bugger"); // its a good idea to test for errors.
...
void init_ogl(Target_State* state, int width, int height)
{
    ...
    win = XCreateWindow(
        state->xDisplay, root, 0, 0, width, height, 0, CopyFromParent, InputOutput, CopyFromParent, CWEventMask, &swa);
    ...
    XSendEvent(state->xDisplay, DefaultRootWindow(state->xDisplay), FALSE, SubstructureNotifyMask, &xev);
...
void Draw(Target_State* p_state)
{
    ...
    glVertexAttribPointer(
        p_state->user_data->texCoordLoc, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat), &RectVertices[3]);
...
{
    if (p_state != NULL) { memset(p_state, 0, sizeof(Target_State)); }
}

void esRegisterDrawFunc(Target_State* p_state, void (*draw_func)(Target_State*)) { p_state->draw_func = draw_func; }

void esMainLoop(Target_State* esContext)
{
    int Counter = 0;
    while (Counter++ < 200) {
        if (esContext->draw_func != NULL) esContext->draw_func(esContext);
...
int main(int argc, char* argv[])
{
    ...
    while (TRUE) {
        ...
        free(OurRawData);
        WhichImage++;
        if (WhichImage == NUMBEROFPICS) WhichImage = 0;
